 ...
 */
public class EncryptConfigFactory implements ConfigFactory {
    private static final Logger logger = LoggerFactory.getLogger(EncryptConfigFactory.class);
    private ConfigUtil m_configUtil;

    public EncryptConfigFactory() {
    ...
    public Config create(String namespace) {
        ...
        if (ConfigFileFormat.isPropertiesCompatible(format)) {
            // 修改为加载自定义的EncryptConfig
            return new EncryptConfig(namespace, createPropertiesCompatibleFileConfigRepository(namespace, format));
        }
        // 修改为加载自定义的EncryptConfig
        return new EncryptConfig(namespace, createLocalConfigRepository(namespace));
    }
