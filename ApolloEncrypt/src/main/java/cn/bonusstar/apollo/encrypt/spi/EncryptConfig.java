package cn.bonusstar.apollo.encrypt.spi;

import cn.bonusstar.apollo.encrypt.service.ApolloCrypto;
 ...
 */
public class EncryptConfig extends AbstractConfig implements RepositoryChangeListener {
     ...
     */
    public EncryptConfig(String namespace, ConfigRepository configRepository) {
    ...
    private void updateConfig(Properties newConfigProperties, ConfigSourceType sourceType) {
        // 先尝试用环境变量内的apollo加密密钥初始化
        ApolloCrypto.initByEnv();

        // 再对所有数据尝试解密
        for (String key : newConfigProperties.stringPropertyNames()) {
            String value = newConfigProperties.getProperty(key);
            String decryptStr = ApolloCrypto.decrypt(value);
            if (!value.equals(decryptStr)) {
                newConfigProperties.put(key, decryptStr);
            }
        }
